...
async def train_lstm_model(request: TrainingRequest):
    """
    Train LSTM model using ONLY cached dataset
    NEVER calls external APIs - fails if dataset not cached
    """
    try:
        ...
        if not data_cache_service.dataset_exists(request.region):
            raise HTTPException(
                status_code=400,
                detail=f"No cached dataset found for {request.region}. Please fetch data first using /api/data/fetch"
            )
        
        # Load cached dataset
        logger.info(f"Loading cached dataset for {request.region}...")
        cached_df = data_cache_service.load_cached_dataset(request.region)
        
        if cached_df.empty:
            raise HTTPException(
                status_code=500,
                detail=f"Cached dataset for {request.region} is empty"
            )
        
        # Train model using ONLY cached data
        logger.info(f"Training LSTM model for {request.region} using cached data ONLY...")
